...
def getMeleeAttack(indexed_combatant, combatants):
    ...
    # New Logic: Filter for melee attacks first
    melee_attacks = [
        atk for atk in atks
        if "Melee" in atk.raw['automation'][1]['text']  # Checking the attack type
    ]
    ranged_attacks = [
        atk for atk in atks
        if "Ranged" in atk.raw['automation'][1]['text']  # Checking the attack type
    ]

    if melee_attacks:
        chosen_atk = str(melee_attacks[randint(len(melee_attacks))])
    elif ranged_attacks:  # Use ranged attack with disadvantage
        chosen_atk = str(ranged_attacks[randint(len(ranged_attacks))])
        chosen_atk += " dis"  # Add "dis" for disadvantage
    else:
        chosen_atk = str(atks[randint(len(atks))])  # Fallback if no clear melee/ranged distinction

    # Existing attack string processing
    chosen_atk_string = str(chosen_atk).split(':')[0][2:-2].casefold()
    if 'Action:' in chosen_atk:
        chosen_atk_string = chosen_atk.split(':')[1:-2][0][:-2]
    chosen_atk_string = resolveVersatile(chosen_atk_string)
...
def getMultiAttacks(mon_actions):
    ...
    if 'Multiattack' in mon_actions:
        ...
        remaining_atk_string = attack_segment.replace('one','1').replace('two','2').replace('three','3').replace('four','4').replace('five','5').replace('six','6').replace('seven','7').replace('and ','')
        while ' with its ' in remaining_atk_string:
            atk_name = remaining_atk_string.split(' with its ')[1].split(',')[0].split(' ')[0].split('.')[0]
            atk_num = remaining_atk_string.split(' with its ')[0][-1]
            remaining_atk_string = remaining_atk_string.replace(f'{atk_num} with its {atk_name}', '').strip()
            atk_name = atk_name[:-1] if atk_name.endswith('s') else atk_name
            multi_atks.update({atk_name: atk_num})

        if not ' with its ' in remaining_atk_string and not multi_atks and ' ' in remaining_atk_string:
            atk_num, atk_name = remaining_atk_string.split(' ')[0], remaining_atk_string.split(' ')[1]
            multi_atks.update({atk_name: atk_num})
        if not multi_atks:
            try:
